...
import cv2
import tqdm
from constant import JSON_DIR, MASK_DIR #constant values

# Create a list which contains every file name in "jsons" folder
json_list = os.listdir(JSON_DIR)

# For every json file   # tqdm used for smart progress bar
for json_name in tqdm.tqdm(json_list):
    ...
    #0'lardan olusan bir matris olusturuyoruz. resim boyutları annotate edilmiş resimlerden geliyor
    mask = np.zeros((json_dict["size"]["height"], json_dict["size"]["width"]), dtype=np.uint8)
    #olusan maske nereye kaydedilecek
